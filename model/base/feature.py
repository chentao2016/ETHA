...
def extract_feat_res_clip(img, backbone, feat_ids, bottleneck_ids, lids):
    r""" Extract intermediate features from Clip-ResNet"""
    feats = []

    img = img.type(backbone.conv1.weight.dtype)
    # Layer 0
    feat = backbone.conv1.forward(img)
    feat = backbone.bn1.forward(feat)
