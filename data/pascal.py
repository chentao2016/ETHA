...
class DatasetPASCAL(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'val':
            query_cam_path = self.cam_val_path + query_name + '.npy'
            support_root_path = self.cam_val_path
        else:
            query_cam_path = self.cam_train_path + query_name + '.npy'
            support_root_path = self.cam_train_path

        query_cam_all = np.load(query_cam_path, allow_pickle=True).item()
        try:
            query_class_idx = np.where(query_cam_all['keys'] == class_sample)[0][0]
        except IndexError:
            print('path', query_cam_path)
            print('keys', query_cam_all['keys'])
            print('class sample', class_sample)
        query_cam = torch.from_numpy(query_cam_all['attn_highres'][query_class_idx]).unsqueeze(0).unsqueeze(0).\
            to(torch.float32)  # original size
        query_cam = F.interpolate(query_cam, (400, 400), mode='bilinear', align_corners=True).squeeze(0).squeeze(0)

        nshot = len(support_names)
        support_cams = []
        for nn in range(nshot):
            support_cam_path = support_root_path + support_names[nn] + '.npy'
            support_cam_all = np.load(support_cam_path, allow_pickle=True).item()
            support_class_idx = np.where(support_cam_all['keys'] == class_sample)[0][0]
            support_cam = torch.from_numpy(support_cam_all['attn_highres'][support_class_idx]).unsqueeze(0).unsqueeze(0).\
                to(torch.float32)  # original size
            support_cam = F.interpolate(support_cam, (400, 400), mode='bilinear', align_corners=True).squeeze(0)
            support_cams.append(support_cam)
        support_cams = torch.cat(support_cams, dim=0)
