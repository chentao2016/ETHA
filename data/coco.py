...
class DatasetCOCO(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'val':
            query_cam_path = self.cam_path + query_name[8:-4] + '.npy'
        else:
            query_cam_path = self.cam_path + query_name[10:-4] + '.npy'
        query_cam_all = np.load(query_cam_path, allow_pickle=True).item()
        query_class_idx = np.searchsorted(query_cam_all['keys'], class_sample)
        ...
        for nn in range(nshot):
            ...
            if self.split == 'val':
                support_cam_path = self.cam_path + support_names[nn][8:-4] + '.npy'
            else:
                support_cam_path = self.cam_path + support_names[nn][10:-4] + '.npy'
            support_cam_all = np.load(support_cam_path, allow_pickle=True).item()
            support_class_idx = np.searchsorted(support_cam_all['keys'], class_sample)
