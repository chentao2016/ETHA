...
class DatasetPASCAL_V2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'val':
            ...
            query_cam = torch.load(query_cam_path).unsqueeze(0).unsqueeze(0)
            ...
            for nn in range(nshot):
                ...
                support_cam = torch.load(support_cam_path).unsqueeze(0).unsqueeze(0)
            ...
            for nn in range(nshot):
                ...
                support_cam = torch.load(support_cam_path).unsqueeze(0).unsqueeze(0)  # 1 50 50
                support_cam = F.interpolate(support_cam, (400, 400), mode='bilinear', align_corners=True).squeeze(0)
                support_cams.append(support_cam)
