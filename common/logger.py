...
class Logger:
    ...
    def log_params(cls, model):
        ...
        for k in model.state_dict().keys():
            ...
            if k.split('.')[0] in ['backbone', 'clip_model', 'ori_pe', 'self_pe', 'pe', 'conv1024_512']:
        ...
        # en_flops = FlopCountAnalysis(model.to(device), (torch.rand(2, 3, 400, 400).cuda(), torch.rand(2, 3, 400, 400).cuda(),
        #                                      torch.rand(2, 400, 400).cuda(), torch.rand(2, 400, 400).cuda(),
        #                                     torch.rand(2, 400, 400).cuda(), torch.rand(2, 400, 400).cuda(),
        #                                                 torch.zeros(2, dtype=torch.long).cuda()))
        #
        # en_total_params = flop_count_table(en_flops)
        # print('encoder flops: \n', en_total_params)
        # print()
